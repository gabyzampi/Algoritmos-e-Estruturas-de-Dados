...
int remover(Fila* fila) {
    if (fila->inicio == NULL) {
        printf("\n > Fila vazia! Nao e possível remover.\n");
        esperarEnter();
        return -1;
    }
    No* temp = fila->inicio;
...
void visualizar(Fila* fila) {
    if (fila->inicio == NULL) {
        ...
        printf("\n > Elementos na fila: \n\n\t");
        while (atual != NULL) {
            printf("%s (%d anos) -> ", atual->nome, atual->idade);
            atual = atual->proximo;
        }
    }
    esperarEnter();
...
Nova função para calcular a media de idade e mostrar; (nova funçao)
